...
Game::AfterMoveState Game::makeMove(const Move& move)
{
...
Move Game::constructMoveFromStr(const std::string& moveDescription) const
{
	Move move{};
...
std::pair<Move, int> Game::findMove(const std::uint8_t depth, const int bestPrevBalance, const GameState& gameState)
{
...
std::pair<Move, int> Game::quiescenceSearch(const std::uint8_t depth, const int bestPrevBalance, const GameState& gameState)
{
	if (m_dpEvaluation.size() == EngineData::maxDpEvaluationSize)
	{
		m_dpEvaluation.clear();
...
bool Game::addGamePosition(const Board& board)
{
	return ++m_gamePositions[board] == GameData::maxRepetitions;
}

void Game::eraseGamePosition(const Board& board)
{
